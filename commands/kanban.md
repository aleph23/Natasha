...
Check if this script is located in the current workspace or in `$HOME/.agents/skillsplans-kanban` directory:
- If in current workspace: `$SKILL_DIR_PATH` = `./.agents/skillsplans-kanban/`
- If in home directory: `$SKILL_DIR_PATH` = `$HOME/.agents/skillsplans-kanban/`
...
```json
{
  "command": "node .agents/skillsplans-kanban/scripts/server.cjs --dir \"./plans\" --host 0.0.0.0 --open --foreground",
