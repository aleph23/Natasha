...
Check if this script is located in the current workspace or in `$HOME/.agents/skillsmarkdown-novel-viewer` directory:
- If in current workspace: `$SKILL_DIR_PATH` = `./.agents/skillsmarkdown-novel-viewer/`
- If in home directory: `$SKILL_DIR_PATH` = `$HOME/.agents/skillsmarkdown-novel-viewer/`
...
```json
{
  "command": "node .agents/skillsmarkdown-novel-viewer/scripts/server.cjs --dir \"path\" --host 0.0.0.0 --open --foreground",
