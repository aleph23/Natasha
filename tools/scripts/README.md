...
4. **PROJECT/.agents/.env** - Project global defaults
5. **~/.agents/skills/\<skill\>/.env** - User skill-specific
6. **~/.agents/skills/.env** - User shared across skills
7. **~/.agents/.env** - User global defaults (LOWEST)

### CLI Usage

```bash
# Resolve a variable for a specific skill
python ~/.agents/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal

# With verbose output
python ~/.agents/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --verbose

# Find all locations where variable is defined
python ~/.agents/scripts/resolve_env.py GEMINI_API_KEY --find-all

# Show hierarchy for a skill
python ~/.agents/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal

# Export format for shell sourcing
eval $(python ~/.agents/scripts/resolve_env.py GEMINI_API_KEY --export)
```
...
if not api_key:
    ...
    print("Run: python ~/.agents/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
    sys.exit(1)
...
python ~/.agents/scripts/resolve_env.py TEST_VAR --verbose
...
python ~/.agents/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --find-all
```
...
python .agents/scripts/generate_catalogs.py --skills

# Generate commands catalog (outputs to stdout)
python .agents/scripts/generate_catalogs.py --commands

# Generate both catalogs (outputs to stdout)
python .agents/scripts/generate_catalogs.py

# Write to file instead of stdout
python .agents/scripts/generate_catalogs.py --skills --output guide/SKILLS.yaml

# View help
python .agents/scripts/generate_catalogs.py --help
```
