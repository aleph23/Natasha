...
Scans .agents/commands/ directory to build catalog at runtime.
...
CATEGORY_GUIDES = {
    ...
    "config": {
        ...
        "workflow": [
            ("Global", "Set user prefs in `~/.agents/.ck.json`"),
            ("Local", "Override per-project in `./.agents/.ck.json`"),
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    """Scan .agents/commands/ and build command catalog."""
    commands = {}
    categories = {}
...
def show_config_guide() -> None:
    ...
    print("- Global: `~/.agents/.ck.json` (user preferences)")
    print("- Local: `./.agents/.ck.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.agents/.ck.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.agents/.ck.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.agents/.ck.json - applies everywhere")
    ...
    print("# Local (.agents/.ck.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.agents/.ck.json` - personal preference")
    print("- Local: `./.agents/.ck.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    ...
    print("2. If 0-5, injects guidelines from `.agents/output-styles/coding-level-*.md`")
    ...
    print("## Customization")
    print()
    print("Guidelines live in `.agents/output-styles/coding-level-*.md`")
...
def main():
    # Find .agents/commands directory
    script_path = Path(__file__).resolve()
    claude_dir = script_path.parent.parent  # .agents/scripts -> .claude
    commands_dir = claude_dir / "commands"

    if not commands_dir.exists():
        print("Error: .agents/commands/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No commands found in .agents/commands/")
        sys.exit(1)
