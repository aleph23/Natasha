...
[ ! -f .gemini/settings.json ] && mkdir -p .gemini && ln -sf .agents/.mcp.json .gemini/settings.json
...
```bash
npx tsx .agents/skills/mcp-management/scripts/cli.ts call-tool <server> <tool> '<json-args>'
```
