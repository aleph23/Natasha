...
Keep `.agents/` directory in version control:
...
- `.agents/skills/`
- `.agents/hooks.json`
- `.agents/mcp.json` (without secrets)
- `.agents/.env.example`

**What NOT to commit:**
- `.agents/.env` (contains secrets)
- `.agents/memory/` (optional)
- `.agents/cache/`
- API keys or tokens
...
echo 'ANTHROPIC_API_KEY=sk-ant-xxxxx' > .agents/.env
```
...
```bash
# Check hooks.json
cat .agents/hooks.json | jq .

# Review scripts
cat .agents/scripts/hook.sh
...
```bash
#!/bin/bash
# .agents/scripts/rate-limit.sh

REQUESTS_FILE=".agents/requests.log"
MAX_REQUESTS=100
WINDOW=3600  # 1 hour
...
```bash
#!/bin/bash
# .agents/scripts/budget-check.sh
