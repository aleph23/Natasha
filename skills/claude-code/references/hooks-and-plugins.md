...
```bash
#!/bin/bash
# .agents/scripts/validate-bash.sh
...
```json
{
  "hooks": {
    "pre-tool": {
      "bash": "./.agents/scripts/validate-bash.sh"
    }
  }
}
```
...
```json
{
  "hooks": {
    "post-tool": {
      "write": "./.agents/scripts/format-code.sh",
      "edit": "./.agents/scripts/format-code.sh"
    }
  }
}
```
...
```bash
#!/bin/bash
# .agents/scripts/track-usage.sh

# Log prompt
echo "$(date): $USER_PROMPT" >> .agents/usage.log
...
{
  "hooks": {
    "user-prompt-submit": "./.agents/scripts/track-usage.sh"
  }
}
```
