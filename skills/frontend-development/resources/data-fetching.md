...
export const MyComponent = { id }: React.ReactElement | null =>
 {
...
export const UserProfile = { userId }: React.ReactElement | null =>
 {
    const { data: user } = useSuspenseQuery({
        queryKey: ['user', userId],
        queryFn: () => userApi.getUser(userId),
...
1. **Create API Service**: `app/X/api/XApi.ts` using apiClient
