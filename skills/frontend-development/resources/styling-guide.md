...
export const componentStyles: Record<string, Props<Theme>> = {
    container: { ... },
    header: { ... },
    // ... 100+ lines of styles
};
...
export const MyComponent = : React.ReactElement | null =>
 {
    return <div sx={componentStyles.container}>...</div>;
};
```
...
```typescript
const formStyles: Record<string, Props<Theme>> = {
    gridContainer: {
        height: '100%',
...
// Type-safe styles
const styles: Record<string, Props<Theme>> = {
    container: {
        p: 2,
...
// Or individual style
const containerStyle: Props<Theme> = {
    p: 2,
    display: 'flex',
};
```
